...
def signup(request):
    if request.method=="POST":
        ...
        if not get_otp:
            ...
            send_mail(
            "welcome to AVF Veryeify your Email",
            mess,
    ...
    if request.method=="POST":
        ...
        if not get_otp:
            ...
            if user is not None:
                ...
                profile.save()
                mess=f'Hello\t{user.username},\nYour OTP to Login your account for AVF is {otp}\nThanks!'
                send_mail(
                "welcome to AVF Verify your Email for login",
                mess,
                settings.EMAIL_HOST_USER,
                [user.email],
                fail_silently=False
     ...
     if request.method=="POST":
        ...
        if not get_otp:
            ...
            if user is not None:
                ...
                profile = Profile(user = user, otp = otp)
                profile.save()
                mess=f'Hello\t{user.username},\nYour OTP to resetting password for AVF account - {otp}\nThanks!'
                send_mail(
                "welcome to AVF Verify your Email for password resetting",
                mess,
                settings.EMAIL_HOST_USER,
                [user.email],
                fail_silently=False
